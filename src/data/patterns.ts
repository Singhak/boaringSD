  ...
  },
  {
    ...
    review: {
      ...
      options: [
        ...
        {
          id: "r-block",
          label: "Hold worker threads open with a 60-second HTTP timeout hoping the dependency recovers",
          isCorrect: false,
          explanation: "Holding request threads open exhausts the server's worker pool and connection backlog, cascading downtime to healthy endpoints.",
        },
        {
          id: "r-retry-storm",
          label: "Retry failing requests immediately in an aggressive tight loop without exponential backoff",
          isCorrect: false,
          explanation: "Immediate retry loops amplify traffic onto an already struggling downstream dependency, triggering a retry storm that guarantees it cannot recover.",
        },
      ],
    },
    tradeoff: {
