...
export const PATTERNS: SystemDesignPattern[] = [
  ...
  },
  {
    ...
    rewards: { firstClearXp: 360, replayXp: 50, builderXp: 180, reviewXp: 55 },
    nextHook: "Core patterns cleared. Next, the Depth tier: during a network partition, every write has to choose between answering and being right.",
    replayFamily: [
      {
        variantId: "baseline",
        label: "Baseline",
  ...
  },
  {
    ...
    levelNumber: 16,
    title: "CAP & PACELC",
    levelGoal: "Choose consistency or availability per feature",
    chapterId: "chapter-16",
    prerequisites: ["health-checks"],
    inherits: ["consistency", "multi-region"],
    difficulty: "Advanced",
    estimatedMinutes: 8,
    skillTags: ["CAP theorem", "PACELC", "network partitions", "CP vs AP", "tunable consistency"],
    newConstraint: "The link between two regions drops. Every replicated write now has to pick: refuse the request, or accept it and risk divergence.",
    objectives: RUN_OBJECTIVES,
    diagnosis: {
      question: "The inter-region link drops for 90 seconds. The EU side can still take requests but cannot reach the US primary. What does the CAP theorem say the EU replicas must do for each write?",
      options: [
        {
          id: "d-choose",
          label: "Either reject the write (stay consistent) or accept it locally and reconcile later (stay available)",
          isCorrect: true,
          explanation: "During a partition you cannot have both: a node that can't reach its peers either refuses (CP) or answers with possibly divergent data (AP). The choice is per operation, not per company.",
        },
        {
          id: "d-both",
          label: "Accept the write and keep it strongly consistent by retrying the US primary in the background",
          isCorrect: false,
          explanation: "Acking a write the primary hasn't seen is exactly giving up consistency: another region can read the old value. Background retries are reconciliation, i.e. the AP choice.",
        },
        {
          id: "d-ignore",
          label: "Nothing special: CAP only matters for systems spread across more than three regions",
          isCorrect: false,
          explanation: "Any replicated system whose nodes can lose contact faces the tradeoff, even two nodes in one datacenter. Partitions are about lost messages, not region count.",
        },
      ],
    },
    intervention: {
      question: "The store has two features on the same replicated database: product reviews and gift-card balances. How should each behave during the partition?",
      options: [
        {
          id: "i-per-feature",
          label: "Reviews stay writable and merge later (AP); gift-card spends reject unless a quorum confirms (CP)",
          isCorrect: true,
          explanation: "A duplicated or late review is harmless; a double-spent gift card is money lost. Pick per feature: availability where divergence is cheap, consistency where it is not.",
        },
        {
          id: "i-all-cp",
          label: "Make every feature CP so no user ever sees stale data during a partition",
          isCorrect: false,
          explanation: "The EU side would stop accepting reviews, carts and logins for the whole outage. You pay unavailability for data that never needed strong consistency.",
        },
        {
          id: "i-all-ap",
          label: "Make every feature AP and resolve all conflicts with last-write-wins timestamps",
          isCorrect: false,
          explanation: "Last-write-wins silently drops one side's update. For a balance, two regions can each approve a $50 spend from a $50 card and one debit vanishes on merge.",
        },
      ],
    },
    transfer: {
      question: "There is no partition today, yet the team debates leader-only reads vs local follower reads for a feed. Which part of PACELC applies?",
      options: [
        {
          id: "t-else",
          label: "The 'Else' half: with no partition you still trade latency for consistency on every read",
          isCorrect: true,
          explanation: "PACELC: if Partition, choose A or C; Else, choose Latency or Consistency. A cross-region leader read costs ~100 ms; a local follower read is fast but may be seconds stale.",
        },
        {
          id: "t-na",
          label: "None: PACELC only describes behaviour while a network partition is happening",
          isCorrect: false,
          explanation: "That is CAP's scope. PACELC exists precisely to cover normal operation, where the cost of consistency is latency rather than availability.",
        },
        {
          id: "t-cap",
          label: "The CAP half: the feed must pick availability because it is read-heavy",
          isCorrect: false,
          explanation: "Read-heavy doesn't imply a partition. Without one, the question is how much latency you pay to read the freshest value.",
        },
      ],
    },
    review: {
      question: "Why is 'we picked CA' not a meaningful answer for a system replicated across machines?",
      options: [
        {
          id: "r-ca",
          label: "Networks do drop messages, so partitions will happen; you only choose how to behave when they do",
          isCorrect: true,
          explanation: "P isn't optional once data lives on more than one machine. 'CA' really means 'we haven't decided what happens during a partition'.",
        },
        {
          id: "r-cost",
          label: "CA is valid but too expensive, since it needs dedicated fibre between every pair of nodes",
          isCorrect: false,
          explanation: "Even dedicated links fail, switches reboot and GC pauses look like partitions. No amount of spend removes P.",
        },
        {
          id: "r-sql",
          label: "Only SQL databases can be CA; NoSQL stores are always AP by design",
          isCorrect: false,
          explanation: "The data model is irrelevant. Many NoSQL stores offer CP modes, and replicated SQL databases face the same partition choice.",
        },
      ],
    },
    tradeoff: {
      whatFailed: "During an inter-region partition, every feature behaved the same way: either everything went read-only or balances diverged.",
      whyFixWorked: "Each feature chose its partition behaviour: AP for data that merges safely, CP (quorum-confirmed) for money and inventory.",
      insufficientWhen: "An AP feature has no merge rule (counters, uniqueness), or CP writes need cross-region quorums on every request and latency budgets can't absorb ~100 ms.",
    },
    builderScenarioId: "boss-cap-pacelc",
    rewards: { firstClearXp: 380, replayXp: 55, builderXp: 190, reviewXp: 60 },
    nextHook: "Next: the CP side needs a leader that every node agrees on, even when the old leader is only mostly dead.",
    replayFamily: [
      {
        variantId: "baseline",
        label: "Per-feature partition choice",
        scenario: "Inter-region link drops; reviews and gift-card balances share one replicated store.",
        constraint: "Money must never double-spend; reviews must stay writable.",
        expectedFix: "AP with merge for reviews, quorum-confirmed CP for balances.",
      },
      {
        variantId: "else-latency",
        label: "No partition, still a tradeoff",
        scenario: "Feed reads go cross-region to the leader and cost 110 ms each.",
        constraint: "Posts may be a few seconds stale.",
        expectedFix: "Serve feed reads from local followers; keep leader reads for read-your-writes paths.",
      },
    ],
  },
  {
    ...
    levelGoal: "Elect one leader and never two",
    chapterId: "chapter-17",
    prerequisites: ["cap-pacelc"],
    inherits: ["cap-pacelc", "health-checks"],
    difficulty: "Advanced",
    estimatedMinutes: 9,
    skillTags: ["Raft", "leader election", "terms", "majority quorum", "split brain", "R+W>N"],
    newConstraint: "Two nodes both believe they are the leader and both accept writes for the same keys.",
    objectives: RUN_OBJECTIVES,
    diagnosis: {
      question: "A 4-node cluster splits 2|2 after a switch failure. Each half promotes its own leader via a 'most recent heartbeat wins' script. What went wrong?",
      options: [
        {
          id: "d-majority",
          label: "Neither half had a majority, yet both elected a leader, so the cluster split-brained",
          isCorrect: true,
          explanation: "A leader must be granted votes by a strict majority (3 of 4). Two halves of 2 cannot both reach 3, so a quorum rule allows at most one leader, or none.",
        },
        {
          id: "d-heartbeat",
          label: "Heartbeats were too slow; a 50 ms interval would have avoided two leaders",
          isCorrect: false,
          explanation: "Faster heartbeats only detect the failure sooner. Without a majority vote, each isolated half still promotes itself.",
        },
        {
          id: "d-clock",
          label: "The two halves' clocks drifted, so their lease timestamps disagreed",
          isCorrect: false,
          explanation: "Clock drift can break lease-based schemes, but here the failure is structural: the election never required a majority.",
        },
      ],
    },
    intervention: {
      question: "You move the store to Raft. How big should the cluster be to tolerate one node failure, and why?",
      options: [
        {
          id: "i-three",
          label: "3 voters: a majority of 2 survives one loss, and a 1|2 split can elect only one side",
          isCorrect: true,
          explanation: "With 2f+1 voters Raft tolerates f failures. 3 nodes tolerate 1; 5 tolerate 2. Adding a 4th voter raises the quorum to 3 without tolerating more failures.",
        },
        {
          id: "i-two",
          label: "2 voters: one leader plus one hot standby is enough for any single failure",
          isCorrect: false,
          explanation: "With 2 voters the majority is 2, so losing either node halts writes. And with no tiebreaker, a partition leaves each side unable to tell 'peer died' from 'link died'.",
        },
        {
          id: "i-four",
          label: "4 voters: the extra node makes the cluster survive two simultaneous failures",
          isCorrect: false,
          explanation: "Majority of 4 is 3, so 4 voters still tolerate only 1 failure, and every commit now needs 3 acks instead of 2.",
        },
      ],
    },
    transfer: {
      question: "A leaderless store keeps N=3 replicas per key. Writes use W=2. What read quorum R guarantees every read overlaps the latest acknowledged write?",
      options: [
        {
          id: "t-r2",
          label: "R=2, because R+W=4 exceeds N=3, so the sets always share a replica",
          isCorrect: true,
          explanation: "Any 2 of 3 read replicas must include at least one of the 2 that took the write. R=1 with W=2 can miss it; R+W>N is the overlap rule.",
        },
        {
          id: "t-r1",
          label: "R=1, because W=2 already stored the write on a majority",
          isCorrect: false,
          explanation: "The one replica you read might be the one the write skipped. R+W=3 is not greater than N=3, so overlap isn't guaranteed.",
        },
        {
          id: "t-r3",
          label: "R=3, because only reading every replica is guaranteed to be fresh",
          isCorrect: false,
          explanation: "R=3 also works but is not required, and a single slow or down replica now fails every read. R=2 is the minimum that overlaps.",
        },
      ],
    },
    review: {
      question: "In Raft, an old leader wakes from a 20 s GC pause and tries to replicate an entry. Why do followers reject it?",
      options: [
        {
          id: "r-term",
          label: "They have moved to a higher term, and any message from a lower term is refused",
          isCorrect: true,
          explanation: "Terms are Raft's logical clock. A new election bumped the term; the stale leader learns it on the first reply and steps down to follower.",
        },
        {
          id: "r-ip",
          label: "Its IP address was removed from the cluster's DNS record during the pause",
          isCorrect: false,
          explanation: "Membership isn't DNS-based. Followers reject it purely because its term number is behind theirs.",
        },
        {
          id: "r-checksum",
          label: "Its log entry checksum no longer matches after the garbage collection pause",
          isCorrect: false,
          explanation: "GC doesn't corrupt entries. The term check is what fences out a leader that no longer holds a majority.",
        },
      ],
    },
    tradeoff: {
      whatFailed: "Leader election without a majority rule let both halves of a partition accept conflicting writes.",
      whyFixWorked: "An odd-sized Raft group requires a majority vote per term and per commit, so at most one leader can make progress.",
      insufficientWhen: "The majority is lost (2 of 3 down), so writes stop by design; or voters span regions and every commit pays a cross-region round trip.",
    },
    builderScenarioId: "boss-consensus-quorums",
    rewards: { firstClearXp: 400, replayXp: 55, builderXp: 200, reviewXp: 60 },
    nextHook: "Next: the log is safe, but the disk underneath it is drowning in random writes.",
    replayFamily: [
      {
        variantId: "baseline",
        label: "Split brain",
        scenario: "A 2|2 split elects two leaders who both accept writes.",
        constraint: "At most one leader may make progress.",
        expectedFix: "Odd-sized Raft group with majority votes and terms.",
      },
      {
        variantId: "quorum-math",
        label: "Leaderless quorums",
        scenario: "N=3, W=1, R=1 and users see their own writes vanish.",
        constraint: "Reads must see the latest acknowledged write.",
        expectedFix: "Choose R and W with R+W>N, e.g. W=2, R=2.",
      },
    ],
  },
  {
    ...
    levelGoal: "Match the engine and indexes to the workload",
    chapterId: "chapter-18",
    prerequisites: ["consensus-quorums"],
    inherits: ["sharding", "caching"],
    difficulty: "Advanced",
    estimatedMinutes: 9,
    skillTags: ["B-tree", "LSM tree", "write amplification", "compaction", "secondary indexes", "Bloom filters"],
    newConstraint: "A write-heavy event log saturates disk on a B-tree database while point lookups for missing keys scan every file.",
    objectives: RUN_OBJECTIVES,
    diagnosis: {
      question: "An IoT ingest writes 80k small rows/s with random UUID keys into a B-tree table. Disk write throughput is maxed but only 20 MB/s of row data arrives. What is happening?",
      options: [
        {
          id: "d-wamp",
          label: "Random inserts dirty a different page each time, so every small row rewrites a whole page",
          isCorrect: true,
          explanation: "B-trees update in place. A ~100-byte row landing in a random 8-16 KB page, plus WAL and page splits, multiplies bytes written many times over: write amplification.",
        },
        {
          id: "d-reads",
          label: "The table has too many reads, which are evicting the buffer pool",
          isCorrect: false,
          explanation: "The workload is write-heavy. The disk is busy with page writes, not with reads.",
        },
        {
          id: "d-cpu",
          label: "UUID generation on the app servers is CPU-bound and throttling the inserts",
          isCorrect: false,
          explanation: "Generating a UUID costs nanoseconds. The bottleneck is the storage layout random keys produce.",
        },
      ],
    },
    intervention: {
      question: "The workload is append-mostly and read-light. Which change fixes the write path at the root?",
      options: [
        {
          id: "i-lsm",
          label: "An LSM engine: buffer in a memtable, flush sorted runs sequentially, compact in the background",
          isCorrect: true,
          explanation: "LSM trees turn random writes into sequential ones. Cost: reads may check several levels, and compaction uses disk and CPU in the background.",
        },
        {
          id: "i-index",
          label: "Add secondary indexes on the device and timestamp columns",
          isCorrect: false,
          explanation: "Every index is another B-tree to update on each insert. More indexes make write amplification worse, not better.",
        },
        {
          id: "i-ram",
          label: "Double the buffer pool RAM so all dirty pages stay in memory",
          isCorrect: false,
          explanation: "Dirty pages still have to be checkpointed to disk. At 80k random inserts/s the working set outgrows RAM within hours.",
        },
      ],
    },
    transfer: {
      question: "On the new LSM store, lookups for device IDs that were never written take 9 ms because they check every level. What is the standard fix?",
      options: [
        {
          id: "t-bloom",
          label: "A Bloom filter per SSTable, so absent keys skip files without a disk read",
          isCorrect: true,
          explanation: "A Bloom filter answers 'definitely not here' or 'maybe'. At ~10 bits per key the false-positive rate is about 1%, so misses rarely touch disk.",
        },
        {
          id: "t-compact",
          label: "Run a full major compaction every hour so only one file remains",
          isCorrect: false,
          explanation: "Full compactions rewrite the whole dataset, adding the very write amplification the LSM was chosen to avoid, and levels rebuild minutes later.",
        },
        {
          id: "t-btree",
          label: "Switch back to a B-tree, which answers missing-key lookups in one seek",
          isCorrect: false,
          explanation: "That reopens the random-write problem the switch solved. Bloom filters keep LSM point misses cheap without giving up sequential writes.",
        },
      ],
    },
    review: {
      question: "When is a B-tree the better engine choice than an LSM tree?",
      options: [
        {
          id: "r-btree",
          label: "Read-heavy workloads needing predictable point and range reads, where writes are moderate",
          isCorrect: true,
          explanation: "B-trees keep each key in one place, so reads are one tree walk with steady latency. LSMs win when writes dominate and compaction cost is acceptable.",
        },
        {
          id: "r-never",
          label: "Never: LSM trees are faster than B-trees for every kind of workload",
          isCorrect: false,
          explanation: "LSMs pay read amplification (multiple levels) and compaction stalls. For read-heavy OLTP, B-trees usually give lower, steadier read latency.",
        },
        {
          id: "r-small",
          label: "Only for tables under 1 GB, since B-trees cannot grow past memory",
          isCorrect: false,
          explanation: "B-trees routinely index multi-terabyte tables; only the upper levels need to stay cached for fast lookups.",
        },
      ],
    },
    tradeoff: {
      whatFailed: "A write-heavy log on a random-key B-tree amplified every small insert into page rewrites and maxed the disk.",
      whyFixWorked: "An LSM engine batched writes into sequential flushes, and Bloom filters kept lookups for absent keys off disk.",
      insufficientWhen: "Compaction can't keep up with ingest (write stalls), or the workload shifts to range-heavy reads across many levels.",
    },
    builderScenarioId: "boss-storage-engines",
    rewards: { firstClearXp: 420, replayXp: 60, builderXp: 210, reviewXp: 65 },
    nextHook: "Depth tier cleared. You can now reason from the partition down to the page on disk.",
    replayFamily: [
      {
        variantId: "baseline",
        label: "Write amplification",
        scenario: "80k random-key inserts/s max out a B-tree's disk at 20 MB/s of payload.",
        constraint: "Ingest must keep up without more disks.",
        expectedFix: "LSM engine with sequential flushes and background compaction.",
      },
      {
        variantId: "bloom",
        label: "Missing-key lookups",
        scenario: "Point lookups for absent keys check every SSTable level.",
        constraint: "Misses must not touch disk.",
        expectedFix: "Per-SSTable Bloom filters.",
      },
    ],
  },
];
